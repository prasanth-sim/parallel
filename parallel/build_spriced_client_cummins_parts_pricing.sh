...
echo "Starting build for [$REPO_NAME] on branch [$CLIENT_BRANCH]..."
...
echo "uilding required backend modules with Maven..."
...
echo "🛠️Running Maven build for [$REPO_NAME]..."
(
    cd "$REPO_DIR" || exit 1
    ...
    if [[ -f "$JAR_PATH" ]]; then
        ...
        echo "No JAR found to copy for $REPO_NAME."
    fi
) || exit 1
